...
class ChatMessengerComponent extends Component
{
    ...
    public bool   $open                  = true;   // always visible in this layout
    public        $friends;
    ...
    public function loadFriends(): void
    {
        ...
        $users = User::where('id', '!=', $userId)->get()->map(
            function ($user) use ($userId, &$usersWithUnread) {
                ...
                // UpdateLastSeen middleware on every request — always
                // accurate even if the presence channel misses an event.
                // OR'd with the live Reverb presence array for instant
                // updates without waiting on the next request.
        ...
        // ── Keep the header's message-icon badge in sync ──────
    ...
    // ─── PRESENCE: update online users from Reverb ────────────────

    #[On('updateOnlineUsers')]
    public function updateOnlineUsers(array $userIds): void
    {
        $this->onlineUserIds = $userIds;
        // Refresh friends so the online status is updated in the list
        $this->loadFriends();
    }
    ...
    public function handleMessageReceived($senderId, $receiverId): void
    {
        ...
        if ($receiverId !== Auth::id()) {
            return; // not addressed to this user at all
        }
    ...
    public function sendSticker(string $emoji): void
    {
        ...
        // Sync this user's OTHER open chat widgets (e.g. main-page
        // messenger) since ->toOthers() above never echoes back to us.
    ...
    {
        return view('livewire.admin.messenger.chat-messenger-component');
    }
}
