<div>
            ...
            <div class="col-xl-8 col-lg-6">
                <div class="d-flex flex-wrap gap-2">
                    <div class="search-box">
                        <input type="text" class="form-control form-control-sm" placeholder="Search roles…" wire:model.live.debounce.300ms="search">
                ...
                <div class="card">
                    <div class="card-body p-0">
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @forelse($roles as $role)
                                        ...
                                        @endphp
                                        <tr class="{{ in_array((string) $role->id, $selectedRoles) ? 'table-active' : '' }}">
                                            <td>
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox" wire:model.live="selectedRoles" value="{{ $role->id }}" @if($isProtected) disabled @endif>
                                                    <label class="form-check-label"></label>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex align-items-center gap-2">
                                                    <span class="badge" style="background: {{ $roleColor }}; width:12px; height:12px; border-radius:4px;"></span>
                                                    <div>
                                            ...
                                            <td>
                                                @if($role->users_count > 0)
                                                    <span class="badge badge-success light border-0">{{ $role->users_count }} user(s)</span>
                                                @else
                                            ...
                                            <td>
                                                <div>
                                                    ...
                                                    <div class="text-muted small">{{ $role->created_at->diffForHumans() }}</div>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="d-flex justify-content-center gap-1">
                                                    <button class="btn btn-sm btn-primary" wire:click="viewRole({{ $role->id }})" title="View">
                                                        <i class="fa-regular fa-eye"></i>
                                                    </button>
                                                    @if(!$isProtected)
                                                        <button class="btn btn-sm btn-warning" wire:click="openEdit({{ $role->id }})" title="Edit">
                                                            <i class="fa-regular fa-pen"></i>
                                                        </button>
                                                    @else
                                                        <button class="btn btn-sm btn-secondary" disabled title="System role – cannot edit">
                                                            <i class="fa-regular fa-lock"></i>
                                                        </button>
                                                    @endif
                                                    @if(!$isProtected)
                                                        <button class="btn btn-sm btn-danger" wire:click="confirmDelete({{ $role->id }})" title="Delete">
                                                            <i class="fa-regular fa-trash"></i>
                                                        </button>
                                                    @else
                                                        <button class="btn btn-sm btn-secondary" disabled title="System role – cannot delete">
                                                            <i class="fa-regular fa-lock"></i>
                                                        </button>
                                                    @endif
                                                </div>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="6" class="text-center py-5">
                                                ...
                                                <p class="text-muted">Try adjusting your search, or create your first role.</p>
                                                <button class="btn btn-primary btn-sm" wire:click="openCreate">
                        ...
                        <div class="row align-items-center p-3">
                            <div class="col-md-6">
                                <span>Showing {{ $roles->firstItem() ?? 0 }}–{{ $roles->lastItem() ?? 0 }} of {{ $roles->total() }} roles</span>
                            </div>
    ...
    @if($showViewModal && $viewingRole)
        ...
        @endphp
        <div class="modal fade show d-block" id="viewRoleModal" tabindex="-1" style="background: rgba(0,0,0,0.5);" wire:ignore.self>
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        @if(!$vIsProtected)
                            <button class="btn btn-primary" wire:click="openEdit({{ $viewingRole->id }}); $set('showViewModal', false)">
    ...
    @if($showRoleModal)
        <div class="modal fade show d-block" id="roleModal" tabindex="-1" style="background: rgba(0,0,0,0.5);" wire:ignore.self>
                ...
                <div class="modal-content">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex align-items-center gap-2">
                                        <input type="color" class="form-control form-control-color" style="width:60px; padding:2px;" wire:model="color" value="{{ $color }}">
                                        <span class="badge" style="background: {{ $color }}; width:30px; height:30px; border-radius:4px;"></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold small">Description <span class="text-muted small fw-normal">— optional</span></label>
                            <input type="text" class="form-control @error('description') is-invalid @enderror" wire:model="description" placeholder="Brief description of this role…">
                            @error('description') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>
                        <div class="mb-3">
                            <label class="form-label fw-bold small">
                                Permissions
                                <span class="badge bg-secondary text-white ms-1">{{ count($selectedPermissions) }} selected</span>
                            </label>
                                            ...
                                            <div class="d-flex flex-wrap gap-1">
                                                @foreach($perms as $perm)
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input" type="checkbox" id="perm_{{ $perm->id }}" wire:model="selectedPermissions" value="{{ $perm->id }}">
                    ...
                    <div class="modal-footer">
                        ...
                        <button class="btn btn-primary" wire:click="saveRole" wire:loading.attr="disabled">
                            <span wire:loading.remove><i class="fa-regular {{ $isEditing ? 'fa-save' : 'fa-plus' }}"></i> {{ $isEditing ? 'Save Changes' : 'Create Role' }}</span>
            ...
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        ...
                        <p>This is <strong>permanent</strong>. Users assigned this role will lose its permissions immediately.</p>
                    </div>
        ...
        <div class="modal fade show d-block" id="bulkDeleteModal" tabindex="-1" style="background: rgba(0,0,0,0.5);" wire:ignore.self>
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    ...
                    <div class="modal-body text-center">
                        <i class="fa-regular fa-shield-slash fa-3x text-danger mb-3"></i>
                        <p>All <strong>{{ count($selectedRoles) }} roles</strong> will be permanently removed. Affected users will lose those permissions immediately.</p>
                    </div>
                    <div class="modal-footer justify-content-center">
