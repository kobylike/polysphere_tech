                ...
                <div class="modal-content border-0 shadow-lg rounded-4">
                    <div class="modal-header border-0 pb-0">
                        ...
                        <button type="button" class="btn-close"
                            @click="open = false; $wire.cancelAvatarDelete()"></button>
                    </div>
                    ...
                    <div class="modal-footer border-0 pt-0 justify-content-center gap-3">
                        <button type="button" class="btn btn-light px-4 rounded-pill"
                            @click="open = false; $wire.cancelAvatarDelete()">Cancel</button>
                        <button type="button" class="btn btn-danger px-4 rounded-pill"
                            @click="$wire.deleteAvatar()">Yes, Remove</button>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <form wire:submit.prevent="save" class="mt-2">
                            ...
                            <div class="col-md-9 col-12">
                                <div class="d-flex flex-wrap align-items-center gap-3">
                                    ...
                                    <div class="flex-grow-1">
                                        <input type="file" class="form-control form-control-sm" wire:model="avatar"
                                            accept="image/*">
                                        @error('avatar') <span
                                        class="text-danger small d-block mt-1">{{ $message }}</span> @enderror
                                        @if($avatar)
                                            <div class="mt-1"><span class="badge bg-success">New avatar selected</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="phone-wrapper position-relative" wire:key="phone-field">
                                    <div class="input-group">
                                        <button type="button" wire:click="toggleCountryDropdown"
                                            ...
                                            <span class="fw-semibold text-dark"
                                                style="font-size: 0.7rem;">{{ $countryCode }}</span>
                                                ...
                                                @forelse($filteredCountries as $country)
                                                    <button type="button"
                                                        ...
                                                        <span class="text-muted flex-shrink-0"
                                                            style="font-size: 0.65rem;">{{ $country['code'] }}</span>
                                                    </button>
                                                @empty
                                                    <div class="px-2 py-2 text-muted text-center" style="font-size: 0.75rem;">No
                                                        countries found</div>
                                                @endforelse
                                            </div>
                                        </div>
                                    ...
                                    @if($phone_local)
                                        <div class="form-text text-primary"
                                            style="font-size: 0.65rem; margin-top: 0.15rem;">
                                            <i class="fas fa-info-circle me-1"></i> Saved as:
                                            <strong>{{ $countryCode }}{{ $phone_local }}</strong>
                                        </div>
                                    @endif
                                    @error('phone_local')
                                        <div class="invalid-feedback d-block" style="font-size: 0.7rem;">{{ $message }}
                                        </div>
                                    @enderror
                                </div>
                            </div>
                        ...
                        @if($user->profile && $user->profile->is_employee)
                            ...
                            <div class="row mb-3 align-items-start">
                                ...
                                <div class="col-md-9 col-12">
                                    <input type="text"
                                        class="form-control form-control-sm mb-2 @error('emergency_contact_name') is-invalid @enderror"
                                    ...
                                    <div class="phone-wrapper position-relative" wire:key="emergency-phone-field">
                                        <div class="input-group">
                                            <button type="button" wire:click="emergency_toggleCountryDropdown"
                                                ...
                                                <img src="{{ asset('flags/' . $emergency_selectedFlag) }}" class="rounded-1"
                                                    style="width: 18px; height: 12px; object-fit: cover; flex-shrink: 0;">
                                                <span class="fw-semibold text-dark"
                                                    style="font-size: 0.7rem;">{{ $emergency_countryCode }}</span>
                                                <i class="fas fa-chevron-down text-muted" style="font-size: 0.5rem;"></i>
                                            </button>
                                            <input type="tel" inputmode="numeric"
                                                wire:model.defer="emergency_contact_phone_local"
                                        ...
                                        @if($emergency_contact_phone_local)
                                            <div class="form-text text-primary"
                                                style="font-size: 0.65rem; margin-top: 0.15rem;">
                                                <i class="fas fa-info-circle me-1"></i> Saved as:
                                                <strong>{{ $emergency_countryCode }}{{ $emergency_contact_phone_local }}</strong>
                                            </div>
                                        @endif
                                ...
                                @foreach($education as $index => $edu)
                                    <div class="row g-1 mb-2 align-items-end" wire:key="edu-{{ $index }}" x-data="{
                                        ...
                                        <div class="col-sm-2 col-6">
                                            <input type="text" class="form-control form-control-sm"
                                                wire:model="education.{{ $index }}.degree" placeholder="Degree">
                                        </div>
                                        <div class="col-sm-2 col-4">
                                            <select class="form-select form-select-sm"
                                                wire:model="education.{{ $index }}.start_year" x-model="startYear">
                                        ...
                                        <div class="col-sm-2 col-4">
                                            <select class="form-select form-select-sm"
                                                wire:model="education.{{ $index }}.end_year" @if($edu['currently_studying'] ?? false) disabled @endif>
                        ...
                        <div class="row">
                            <div class="col-md-9 offset-md-3 col-12">
                                <button type="submit" class="btn btn-primary btn-sm w-100 w-sm-auto px-4"
                                ...
                                <a href="{{ route('account', ['tab' => 'overview']) }}" wire:navigate
                                    class="btn btn-white btn-sm ms-2">Cancel</a>
                            </div>
                        </div>
                    </form>
                </div>
            ...
            <div class="card border-0 shadow-sm h-100">
                ...
                <div class="card-body">
                    <ul class="list-unstyled">
                        <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i> Add a professional avatar
                        </li>
                        <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i> Write a compelling "About
                            Me"</li>
                        <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i> List your top skills</li>
                        <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i> Share your education
                            background</li>
                        <li class="mb-2"><i class="fas fa-check-circle text-success me-2"></i> Connect your professional
                            social profiles</li>
                    </ul>
